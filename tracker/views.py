...
def serialize_task(t,d):
 ...
 return {"id":t.id,"title":t.title,"description":t.description,"priority":t.priority,"daily":t.is_daily,"dueDate":t.due_date.isoformat(),"completed":Completion.objects.filter(task=t,date=d,completed=True).exists(),"xp":XP[t.priority],"subtaskXp":share,"subtasks":[{"id":s.id,"title":s.title,"completed":s.completed,"xp":share} for s in subs]}
...
def mission_plan(day,level):
 ...
 return {"title":f"{MISSION_TITLES[index%10]} #{index+1}","flavor":MISSION_FLAVORS[(index//10)%10],"study_minutes":min(240,base_minutes+tier*15),"easy_target":easy+tier//3,"legendary_target":legend+(1 if tier>=4 and index%4==0 else 0),"reward_xp":70+tier*20+index%5*5,"penalty_xp":40+tier*20}
def daily_mission(today,p,minutes,counts):
 ...
 m,_=DailyMission.objects.get_or_create(date=today,defaults={k:plan[k] for k in ["study_minutes","easy_target","legendary_target","reward_xp","penalty_xp"]})
 met=minutes>=m.study_minutes and counts["low"]>=m.easy_target and counts["legendary"]>=m.legendary_target
 if met and not m.completed:m.completed=True;m.save(update_fields=["completed"]);add_xp(p,m.reward_xp,f"Completed daily mission for {today:%b %d}")
 return m
@csrf_exempt
def dashboard(r):
 ...
 for o in range(6,-1,-1):
  ...
  week.append({"date":day.strftime("%a"),"xp":cs.aggregate(x=Sum("points_awarded"))["x"] or 0,"done":cs.filter(completed=True).count()});timeweek.append({"date":day.strftime("%a"),"minutes":round(secs/60)})
 ...
 recommendation=max(stats,key=lambda x:x["focus"],default=None);active=InventoryItem.objects.filter(active_until__gt=timezone.now(),quantity=0).order_by("active_until").first()
 ...
 return JsonResponse({"profile":{**level,"rank":rank_title(level["level"]),"name":p.name,"streak":streak,"bestStreak":p.longest_streak,"dailyGoal":p.daily_focus_goal},"tasks":todo,"summary":{"total":len(todo),"completed":sum(t["completed"] for t in todo),"rate":round(sum(t["completed"] for t in todo)/len(todo)*100) if todo else 0},"week":week,"timeWeek":timeweek,"study":{"todayMinutes":round(today_secs/60),"totalMinutes":round(total_secs/60)},"timer":timer_data(),"boost":{"active":bool(active),"until":active.active_until.isoformat() if active else None},"mission":{"title":current_plan["title"],"flavor":current_plan["flavor"],"studyMinutes":mission.study_minutes,"easyTarget":mission.easy_target,"legendaryTarget":mission.legendary_target,"easyDone":counts["low"],"legendaryDone":counts["legendary"],"reward":mission.reward_xp,"penalty":mission.penalty_xp,"completed":mission.completed},"upcomingMissions":upcoming,"taskStats":stats,"recommendation":recommendation,"activity":[{"amount":x.amount,"reason":x.reason,"kind":x.kind,"at":x.created_at.strftime("%b %d, %H:%M")} for x in ActivityLog.objects.order_by("-created_at")[:12]],"badges":badges,"distractions":Distraction.objects.filter(resolved=False).count()})
@csrf_exempt
def tasks(r):
 if r.method=="GET":
 ...
 if r.method=="POST":
  ...
  t=Task.objects.create(title=title,description=x.get("description","").strip()[:360],priority=x.get("priority") if x.get("priority") in XP else "normal",is_daily=bool(x.get("daily")),due_date=due);return JsonResponse({"task":serialize_task(t,timezone.localdate())},status=201)
